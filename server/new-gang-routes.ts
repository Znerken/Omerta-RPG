...
gangRouter.post("/:id/members/:userId/promote", isAuthenticated, async (req: Request, res: Response) => {
  try {
    ...
    // Update rank
    const updatedMember = await gangStorage.updateMemberRank(targetUserId, gangId, newRank);
    if (!updatedMember) {
...
gangRouter.post("/:id/transfer-leadership/:userId", isAuthenticated, async (req: Request, res: Response) => {
  try {
    ...
    try {
      ...
      await gangStorage.updateMemberRank(req.user.id, gangId, "Underboss");
      
      // Promote target to Leader
      await gangStorage.updateMemberRank(targetUserId, gangId, "Leader");
